...
```
references/<filename>   →   .claude/skills/drafting-resume-from-confirmed-assets/references/<filename>
```

For example:
- `references/latex-template.tex` → `.claude/skills/drafting-resume-from-confirmed-assets/references/latex-template.tex`
- `references/canonical-rules.md` → `.claude/skills/drafting-resume-from-confirmed-assets/references/canonical-rules.md`

If you `cat references/foo.md` from the repo root and get "No such file or directory", **you have the wrong path** — prepend `.claude/skills/drafting-resume-from-confirmed-assets/` and try again. Do not waste tokens searching for these files with `rg` or `find`; the location is fixed.
